			...
			<view>
				<view class="flex list">
					...
					<view class="info flex1">
						<view class="title address">[收货地址]{{addr}}</view>
					</view>
				</view>
...
export default {
	...
	onLoad(options) {
		this.orderExpressNo = options.order_express_no;
		this.user_id = options.user_id;
		this.order_id = options.order_id;
		this.getOrderDeliveryData();this.user_id = options.user_id;	},
	methods: {
		async getOrderDeliveryData() {
			this.tui.request("/user/order/getOrderDeliveryData", "POST", {
				user_id: this.user_id,
				order_express_no: this.orderExpressNo,
